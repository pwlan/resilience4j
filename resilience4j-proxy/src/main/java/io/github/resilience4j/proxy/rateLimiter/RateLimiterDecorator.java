 ...
 */
class RateLimiterDecorator implements ProxyDecorator {

    private final RateLimiter rateLimiter;

    RateLimiterDecorator(RateLimiter rateLimiter) {
    ...
    public CheckedFunction1<Object[], ?> decorate(CheckedFunction1<Object[], ?> fn, Method method) {
        if (isAsync(method)) {
            return (args) -> decorateCompletionStage(rateLimiter, toSupplier(fn, args)).get();
        }
        return decorateCheckedFunction(rateLimiter, fn);
    }
}
